...
@st.cache_data(show_spinner = False)
def CACHE_PrefilteringPoints(с_img, c_params):
    # img_med = PreprocessingMedian(с_img, c_params['sz_med'])
    # img_med_th = PreprocessingTopHat(img_med, c_params['sz_th']) 
    
    lm, _ = PrefilteringPoints(с_img, c_params['min_dist'], c_params['thr_br'])
    print(f"points: {_}")
